...
export function usePDXPredictionMarket() {
    ...
    const createMarket = useCallback(async (
        ...
        try {
            ...
            const totalPDX = initialYesWei + initialNoWei

            // Check if approval is needed
            const currentAllowance = await (pdxToken as any).allowance(account, PDX_PREDICTION_MARKET_ADDRESS)
            if (currentAllowance < totalPDX) {
                await approvePDX(ethers.formatEther(totalPDX))
            }
    ...
    const getMarket = useCallback(async (marketId: number): Promise<PDXMarket> => {
        ...
        try {
            ...
            // Market struct: creator, question, category, endTime, status, outcome, yesToken, noToken,
            //                yesPool, noPool, totalBacking, platformFees, resolutionRequestedAt,
            //                resolutionRequester, resolutionReason, resolutionConfidence, disputeDeadline,
            ...
            const market: PDXMarket = {
                ...
                totalBacking: ethers.formatEther(marketData[10] || 0),
                platformFees: ethers.formatEther(marketData[11] || 0),
                resolutionRequestedAt: Number(marketData[12] || 0),
                resolutionReason: marketData[14] || '',
                resolutionConfidence: Number(marketData[15] || 0),
                disputeDeadline: Number(marketData[16] || 0),
                yesMultiplier: Number(multipliers[0]) / 1_000_000,
                noMultiplier: Number(multipliers[1]) / 1_000_000,
    ...
    const createStopLossOrder = useCallback(async (
        ...
        if (event) {
            const iface = new ethers.Interface(PDX_PREDICTION_MARKET_ABI)
            const decodedEvent = iface.parseLog(event)
            return Number(decodedEvent?.args[0])
        }
    ...
    const getUserOrders = useCallback(async (userAddress?: string): Promise<PDXOrderInfo[]> => {
        ...
        try {
            ...
            for (const orderId of orderIds) {
                ...
                // Order struct: orderId, user, marketId, isYes, tokenAmount, triggerPrice, orderType, isActive, createdAt
                const orderType = Number(orderData[6]) // 0 = StopLoss, 1 = TakeProfit
                orders.push({
                    orderId: Number(orderData[0]),
                    user: orderData[1],
                    marketId: Number(orderData[2]),
                    isYes: orderData[3],
                    tokenAmount: ethers.formatEther(orderData[4]),
                    stopLossPrice: orderType === 0 ? Number(orderData[5]) : 0,
                    takeProfitPrice: orderType === 1 ? Number(orderData[5]) : 0,
