...
contract PDXPredictionMarket is IPDXPredictionMarket {
    ...
    function buyYesWithPDXFor(
        ...
        uint256 minYesOut,
        uint256 pdxAmount
    ) external override nonReentrant marketExists(id) {
    ...
    function _buyWithPDX(
        ...
        uint256 totalFee = (pdxAmount * 300) / 10000; // 3% total
        uint256 creatorFee = (pdxAmount * 200) / 10000; // 2% to creator
        uint256 platformFee = totalFee - creatorFee; // 1% to platform
        uint256 amountToPool = pdxAmount - totalFee; // 97% to pool
    ...
    function executeOrder(uint256 orderId) external override nonReentrant {
        ...
        require(
            m.status == MarketStatus.Open && block.timestamp < m.endTime,
            "market closed"
        );
        ...
        require(
            tokenIn.balanceOf(order.user) >= order.tokenAmount,
            "insufficient balance"
        );
    ...
    function createMarket(
        ...
        require(
            bytes(question).length > 0 && bytes(question).length <= 280,
            "invalid question"
        );
        ...
        require(
            IPDX(pdxToken).transferFrom(msg.sender, address(this), totalPDX),
            "PDX transfer failed"
        );
    ...
    function claimRedemption(
        ...
        if (m.outcome == Outcome.Yes) {
            ...
            require(
                IPDX(pdxToken).transfer(msg.sender, yesTokens),
                "PDX transfer failed"
            );
            ...
            require(
                IPDX(pdxToken).transfer(msg.sender, noTokens),
                "PDX transfer failed"
            );
    ...
    function transferMarketOwnership(
        ...
        emit MarketOwnershipTransferred(
            id,
            previousOwner,
            newOwner,
            block.timestamp
...
contract PDXTradingBot {
    ...
    constructor(address _predictionMarket, address _pdxToken) {
        require(
            _predictionMarket != address(0) && _pdxToken != address(0),
            "zero address"
        );
    ...
    function autoBuyYes(
        ...
        require(
            IPDX(pdxToken).approve(address(predictionMarket), pdxAmount),
            "approval failed"
        );
        predictionMarket.buyYesWithPDXFor(
            marketId,
            beneficiary,
            minYesOut,
            pdxAmount
    ...
    function autoBuyNo(
        ...
        predictionMarket.buyNoWithPDXFor(
            marketId,
            beneficiary,
            minNoOut,
            pdxAmount
    ...
    function getMarketData(
        uint256 marketId
    ) external view returns (IPDXPredictionMarket.MarketInfo memory) {
    ...
    function batchMonitorOrders(
        ...
        for (uint256 i = 0; i < orderIds.length; i++) {
            IPDXPredictionMarket.OrderInfo memory orderInfo = predictionMarket
